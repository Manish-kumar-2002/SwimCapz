...
class CartController extends FrontBaseController
{
    ...
    public function cartNameRemove($id)
    {
        Cart::where('id', $id)
                    ->update(['names' => null]);
        
        return back()
                    ->with('success',  __('Names removed from this product successfully.'));
        
    }

    public function downloadCartNameList($id)
    {
        ...
        $sheet->setCellValue("B$tr", 'Quantity');
        $sheet->getStyle("A$tr:B$tr")->getFont()->setBold(true);
        $tr +=1;

        $datas =Cart::find($id);
        foreach($datas->names as $row) {
            $sheet->setCellValue("A$tr", $row['name']);
            $sheet->setCellValue("B$tr", $row['quantity']);
    ...
    public function addcart($id)
    {
        ...
        ->first([
            'id',  'slug', 'name', 'photo', 'size',
            'size_qty', 'size_price', 'color', 'price',
            'stock', 'type', 'file', 'link', 'license', 'license_qty',
            'measure', 'whole_sell_qty', 'whole_sell_discount',
            'attributes', 'size_all', 'color_all']);
            ...
            if (!empty($cart->items[$id . $size . $color . str_replace(str_split(' ,'), '', $values)])) {
                ...
                if ($cart->items[$id . $size . $color . str_replace(str_split(' ,'), '', $values)]['qty'] < $minimum_qty) {
                    return redirect()->route('front.cart')
                        ->with('unsuccess', __('Minimum Quantity is:') . ' ' . $prod->minimum_qty);
                }
            } else {
                $minimum_qty = (int)$prod->minimum_qty;
    ...
    {
        Helper::manageCart();//manage cart
        try {
            Cart::updateOrCreate([
                'session_id' => Helper::getUserSession(),
                'product_id'        =>$request->id,
                // 'product_varient_id'=>$request->varient,
            ], [
                'user_id'           =>Auth::check() ? Auth::user()->id : null
            ]);

            //TODO:: create modified list
            return response()->json(array('status' =>200, 'message' => 'Added to cart.'));

        }catch (Exception $e) {
            return response()->json(array('status' =>400, 'message' => 'Your request has failed.','error'=>$e));
        }

    }

    private function mainCartOrderDetails($request)
    {
        Helper::reCalculateCart();
        if ($request->has('cart_details_id')) {
            $cartDetails=CartDetails::find($request->cart_details_id);
            return view('frontend._assets._mainCart', [
                'cartDetails' => $cartDetails,
                'cart'        => $cartDetails->cart
            ]);
        }

        return view('frontend._assets._main_cart_order_details');
    }
    public function cartDetailsUpdate(Request $request)
    {
        if($request->has('mainCartOrderDetails')) {
            return $this->mainCartOrderDetails($request);
        }
        ...
        try {
            ...
            $cartDetails=CartDetails::find($request->cart_details_id);
            $cartDetails->total_qty =$request->qty;
            $cartDetails->remarks   =$request->remarks;
            $cartDetails->save();

            $minimumOrderedQty=$cartDetails->productVarients->minimum_order;
            if ($cartDetails->total_qty < $minimumOrderedQty) {
                throw new Exception(
                    __('Order quantity must be greater than minimum order quantity :' .$minimumOrderedQty));
            }

            /* validate */
            $totalNames=$cartDetails->cart->getTotalNames();
            $usedTotalQty=CartDetails::where('cart_id', $cartDetails->cart->id)
                                            ->sum('total_qty');
            if ($usedTotalQty < $totalNames) {
                throw new Exception(__('Order quantity cannot be less than total names\' quantity'));
            }
    ...
    public function _addnumcart(Request $request)
    {
        $id = $_GET['id'];
        $qty = $_GET['qty'];
        ...
        $prod = Product::
                where('id', '=', $id)
                ->first([
                    'id', 'user_id', 'slug', 'name', 'photo', 'size',
                    'size_qty', 'size_price', 'color', 'price', 'stock',
                    'type', 'file', 'link', 'license', 'license_qty',
                    'measure', 'whole_sell_qty', 'whole_sell_discount',
                    'attributes', 'minimum_qty', 'stock_check', 'size_all', 'color_all'
                ]);
    ...
    public function addtonumcart(Request $request)
    {
        ...
        $prod = Product::where('id', '=', $id)->first([
            ...
            'color', 'price', 'stock', 'type', 'file', 'link', 'license', 'license_qty',
            'measure', 'whole_sell_qty', 'whole_sell_discount', 'attributes', 'minimum_qty',
            'stock_check', 'size_all', 'color_all'
        ]);
        if ($prod->type != 'Physical') {
            $qty = 1;
        }
    ...
    {
        try {
            $cartDetails=Cart::findOrFail($id);
            if ($cartDetails) {
        ...
        return back()
            ->with('success', __('Item removed from cart successfully.'));
    }

    /* load mini cart */
    public function loadMiniCart()
    {
        return view('frontend._assets._commonHeaderCart');
    }
