...
class ProductDetailsController extends FrontBaseController
{
    ...
    public function product(Request $request, $slug,$variant_id)
    {
        $affilate_user = 0;
        $gs = $this->gs;
        ...
        $product_click->save();
        $variant_details = ProductVariant::with('variantImages')->findOrFail($variant_id);

        if ($request->has('color-change')) {
            return view('partials.product-details._topAsset',
                compact('productt', 'curr', 'affilate_user', 'recent_products', 'variant_details'));
        }
