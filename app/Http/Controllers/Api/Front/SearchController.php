...
class SearchController extends Controller
{
	...
	public function attributes(Request $request, $id)
	{
		try {
			...
			if ($type == 'childcategory') {
				$attributes = Attribute::where('attributable_id', $id)
					->where('attributable_type', 'App\Models\Childcategory')->get();
			}
	...
	public function attributeoptions($id)
	{
		try {
			...
			return response()->json([
				'status' => true, 
				'data' => AttributeOptionResource::collection($attributeOpts), 'error' => []]);
		} catch (\Exception $e) {
			return response()->json(['status' => true, 'data' => [], 'error' => ['message' => $e->getMessage()]]);
		}
	}
	...
	public function search(Request $request)
	{
		try {
			...
			return response()->json([
				'status' => true,
				'data' => ProductlistResource::collection($prods->flatten(1)), 'error' => []]);
		} catch (\Exception $e) {
	...
	public function searchProducts(Request $request)
  	{
		$products=Product::with('variant')->where('status',true);
