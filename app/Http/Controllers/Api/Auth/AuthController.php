...
class AuthController extends Controller
{
  ...
  public function login(Request $request)
  {
    try {
      ...
      if (auth()->user()->email_verified == 'No') {
        auth()->logout();
        return response()->json(['status' => false, 'data' => [], 'error' => ["message" => ' Verified!']]);
      }
  ...
  public function forgot_submit(Request $request)
  {
    ...
    if ($request->new_password != $request->confirm_password) {
      return response()->json(['status' => false, 'data' => [], 'error' => 'New password does not match with confirm password.']);
    }
