    ...
    public function datatables()
    {
        ...
        return DataTables::of($datas)
            ...
            ->addColumn('status', function (Country $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-country-status', [$data->id, 1]) . '" ' . $s . '>Active</option><option data-val="0" value="' . route('admin-country-status', [$data->id, 0]) . '" ' . $ns . '>Inactive</option>/select></div>';
            })
