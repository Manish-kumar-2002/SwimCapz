...
class PageController extends AdminBaseController
{
    ...
    public function datatables()
    {
        ...
        return Datatables::of($datas)
            ->addColumn('header', function (Page $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-page-header', [$data->id, 1]) . '" ' . $s . '>' . __('Show') . '</option><option data-val="0" value="' . route('admin-page-header', [$data->id, 0]) . '" ' . $ns . '>' . __('Hide') . '</option></select></div>';
            })
            ->addColumn('footer', function (Page $data) {
                ...
                return '<div class="action-list"><select class="process select droplinks ' . $class . '"><option data-val="1" value="' . route('admin-page-footer', [$data->id, 1]) . '" ' . $s . '>' . __('Show') . '</option><<option data-val="0" value="' . route('admin-page-footer', [$data->id, 0]) . '" ' . $ns . '>' . __('Hide') . '</option>/select></div>';
            })
            ->addColumn('action', function (Page $data) {
                return '<div class="action-list"><a href="' . route('admin-page-edit', $data->id) . '"> <i class="fas fa-edit"></i></a><a href="javascript:;" data-href="' . route('admin-page-delete', $data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
            })
    ...
    public function store(Request $request)
    {
        ...
        $details = preg_replace('/<style\b[^>]*>(.*?)<\/style>/is', '', $request->input('details'));
        $details = preg_replace('/style="[^"]*"/i', '', $details);

        // Remove all HTML tags but keep the content
        $strippedDetails = strip_tags($details);

        // Trim white spaces
        $strippedDetails = trim($strippedDetails);

        if (empty($strippedDetails)) {
            return response()->json(array('errors' => ['details' => ['The Description field is required.']]));
        }
        //--- Validation Section
        $slug = $request->slug;
