    ...
    public function datatables($country)
    {
         ...
         return DataTables::of($datas)
                            ...
                            ->addColumn('status', function(State $data) {
                              ...
                              return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('admin-state-status',[ $data->id,1]).'" '.$s.'>Active</option><option data-val="0" value="'. route('admin-state-status',[ $data->id, 0]).'" '.$ns.'>Inactive</option>/select></div>';
                          })
