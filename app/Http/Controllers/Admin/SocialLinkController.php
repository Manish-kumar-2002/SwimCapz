    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ->addColumn('status', function(SocialLink $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('admin-sociallink-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.__("Active").'</option><<option data-val="0" value="'. route('admin-sociallink-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.__("Inactive").'</option>/select></div>';
                            })
                            ->addColumn('action', function(SocialLink $data) {
                                return '<div class="action-list"><a href="' . route('admin-sociallink-edit',$data->id) . '"> <i class="fas fa-edit"></i></a><a href="javascript:;" data-href="' . route('admin-sociallink-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            })
