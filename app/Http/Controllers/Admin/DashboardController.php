...
class DashboardController extends AdminBaseController
{
    ...
    {
        $data['pending']=Order::pendingOrders();
        $data['success']=Order::successOrders();
        $data['accepted']=Order::acceptedOrders();
        $data['rejected']=Order::rejectedOrders();
        $data['shipped']=Order::shippedOrders();
        $data['completed'] = Order::completedOrders();
        ...
        for ($i = 0; $i < 30; $i++) {
            ...
            $data['sales'] .=  "'" . Order::where('order_status', '=', Order::ORDER_COMPLETED)
                ...
                if ($request->newpass == $request->renewpass) {
                        if (strlen($request->newpass) < 8) {
                            return response()->json(['errors' => [
                                0 => __('Password should be at least 8 characters long.')
                            ]]);
                        }
                        else
                        {
                    ...
                    return response()->json(array(
                        'errors' => [0 => __('New password does not match with confirm password.')]));
                }
            } else {
