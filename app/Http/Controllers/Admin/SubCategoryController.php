...
class SubCategoryController extends AdminBaseController
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Subcategory $data) {
                                ...
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('admin-subcat-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.__("Active").'</option><<option data-val="0" value="'. route('admin-subcat-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.__("Inactive").'</option>/select></div>';
                            })
    ...
    public function store(Request $request)
    {
        ...
        //--- Redirect Section
        $msg = __('Sub category Added Successfully.');
    ...
    public function status($id1,$id2)
    {
        ...
        $data->update();
        //--- Redirect Section
        $msg = __('Sub category status Updated Successfully.');
        return response()->json($msg);
        //--- Redirect Section Ends
    }
    ...
    public function destroy($id)
    {
        ...
        $data->delete();
        //--- Redirect Section
        $msg = __('Sub category  Deleted Successfully.');
        return response()->json($msg);
        //--- Redirect Section Ends
    }
}
