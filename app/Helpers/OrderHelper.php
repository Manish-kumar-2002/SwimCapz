...
class OrderHelper
{
    ...
    /* manage low inventory stock notification */
    public static function stockNotifications()
    {
        
        $list=ProductVariant::where('quantity', '<=', ProductVariant::MINIMUM_STOCK_ALERT)
                        ->whereNotIn('id', Notification::where('product_varient', '!=', null)
                        ->pluck('product_varient')->toArray())
                        ->get();

        foreach ($list as $row) {

            $notification = new Notification;
            $notification->product_id = $row->product_id;
            $notification->product_varient = $row->id;
            $notification->save();
