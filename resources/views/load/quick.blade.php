                ...
                <div class="col-lg-7">
                    <div class="product-info">
                        ...
                        <div class="top-meta">
                            ...
                            <div class="review">
                                <i class="far fa-comments"></i> {{ App\Models\Rating::ratingCount($product->id) }}
                                {{ __('Review') }}
                            </div>
                            ...
                            <div class="wish">
                                ...
                                @if (Auth::check())
                                    <a class="add-to-wish" href="javascript:;"
                                        data-href="{{ route('user-wishlist-add', $product->id) }}" data-toggle="tooltip"
                                        data-placement="top" title="{{ __('Wish') }}">
                                        <i class="far fa-heart"></i>
                                    </a>
                                @else
                                    <a rel-toggle="tooltip" href="javascript:;" title="{{ __('Wish') }}"
                                        data-placement="top" class="add-to-wish" data-toggle="modal"
                            ...
                            <div class="compear">
                                ...
                                <a class="add-to-compare" href="javascript:;"
                                    data-href="{{ route('product.compare.add', $product->id) }}" data-toggle="tooltip"
                                    data-placement="top" title="{{ __('Compare') }}">
                                        ...
                                        @foreach (array_unique($product->size) as $key => $data1)
                                            <li class="{{ $loop->first ? 'active' : '' }}"
                                                ...
                                                <span class="box">
                                                    ...
                                                    <input type="hidden" class="msize_key"
                                                        value="{{ $key }}">
                                                </span>
                                            </li>
                                        @endforeach
                                ...
                                <div class="mproduct-color">
                                    ...
                                    <ul class="color-list">
                                        ...
                                        @foreach ($product->color as $key => $data1)
                                            <li
                                                class="{{ $loop->first ? 'active' : '' }} {{ $product->IsSizeColor($product->size[$key]) ? str_replace(' ', '', $product->size[$key]) : '' }} {{ $product->size[$key] == $product->size[0] ? 'show-colors' : '' }}">
                                                <span class="box" data-color="{{ $product->color[$key] }}"
                                                    style="background-color: {{ $product->color[$key] }}">
                                                    <input type="hidden" class="msize"
                                                        value="{{ $product->size[$key] }}">
                                                    <input type="hidden" class="msize_qty"
                                                        value="{{ $product->size_qty[$key] }}">
                                                    <input type="hidden" class="msize_key"
                                                        value="{{ $key }}">
                        ...
                        <input type="hidden" id="mproduct_price"
                            value="{{ round($product->vendorPrice() * $curr->value, 2) }}">
                                ...
                                <div class="cart-btn">
                                    <ul class="btn-list">
                                            ...
                                            <li>
                                                <div class="multiple-item-price">
                                                    <div class="qty">
                                                        ...
                                                        <input class="modal-total" type="text" id="order-qty1"
                                                            value="{{ $product->minimum_qty == null ? '1' : (int) $product->minimum_qty }}">
                                                        <input type="hidden" id="mproduct_minimum_qty"
                                        ...
                                        @if ($product->product_type == 'affiliate')
                                            <li>
                                                <a href="{{ route('affiliate.product', $product->slug) }}"
                                                    target="_blank">
                        ...
                        <div class="mt-2">
                            <a class="view_more_btn"
                                href="{{ route('front.product', $product->slug) }}">{{ __('Get More Details') }} <i
...
<script type="text/javascript">
    (function($) {
        ...
        var sizes = "";
        var size_qty = ($('.mproduct-color .color-list li.active').length > 0) ? parseFloat($(
            '.mproduct-color .color-list li.active').find('.msize_qty').val()) : '';
        var size_price = "";
        var size_key = "";
        var colors = "";
        var total = "";
        ...
        $(document).on("click", "#mqaddcrt", function() {
            ...
            window.location = mainurl + "/addtonumcart?id=" + pid + "&qty=" + qty + "&size=" + sizes +
                "&color=" + colors.substring(1, colors.length) + "&size_qty=" + size_qty + "&size_price=" +
                size_price + "&size_key=" + size_key + "&keys=" + keys + "&values=" + values + "&prices=" +
                prices;
