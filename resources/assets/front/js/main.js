(function ($) {
	...
	$(document).ready(function () {
		...
		$(document).on("submit", ".subscribeformHome", function (e) {
			var $this = $(this);
			e.preventDefault();
			e.stopImmediatePropagation;
			
			let email=$this.find('#subscribe-email').val();
			if(!email) {
		...
		{
			$('.loader').show();
			$.get('/load-mini-cart', function(response) {
				$('._commonHeaderCart').html(response);
				$('.loader').hide();
			});
		}
		...
		$(document).on("click", "#addcrt", function () {
			...
			let varientProduct = $("#varient_id").val();
			$.ajax({
				type: "GET",
